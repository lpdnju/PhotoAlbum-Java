...
description: Generate report.html from assessment report data by running the generate_report_html.py script
---
...
```bash
python .github/modernize/assessment/engines/generate_report_html.py .github/modernize/assessment/reports/report-{reportId}
```

If the script is not at that path, look for it in the skill's own directory or copy it from there. The script file is `generate_report_html.py` bundled alongside this skill.

Alternatively, if the script is available in the current working directory or skill resources:

```bash
python generate_report_html.py .github/modernize/assessment/reports/report-{reportId}
```
